# Habit Quest: Path to Liberation

A production-ready Android habit tracker and personal discipline RPG built with React, Vite, and Capacitor.

## Features
- Daily Habit Tracking with Difficulty Levels
- Spiritual RPG Evolution (7 Saint Levels, 7 Environments)
- XP System with Bonuses
- Discipline Punishment System
- Shop, Achievements, and Quests
- Calendar, Reports (SVG Charts), Mood Tracker, Journal
- PWA & Offline Support
- Capacitor Local Notifications

## Setup
1. `npm install`
2. `npm run dev` (Web)
3. `npx cap sync` && `npx cap open android` (Android)

## Build
`npm run build`
